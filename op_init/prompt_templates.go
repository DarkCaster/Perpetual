...
const defaultAIAnnotatePrompt_CSHTML = "Create a summary for the Razor view file in my next message. It should be as brief as possible, without unnecessary language structures. The summary should include a brief general description of the view's purpose, the main HTML structure and layout, and the key C# logic or data binding operations it performs. Skip private methods and local variables from listing completely.\n\nIf there are comments in the file marked as notes for creating this summary, follow them strictly. Otherwise, use the following template:\n\nThis view provides <description of what this view is used for, up to 3 sentences>\n\nC# Logic:\n- `<method name>`: <summary>\n\nHTML Structure:\n- `<element name>`: <summary>"

const defaultAIAnnotatePrompt_CSHTML_SHORT = "Create a short summary for the Razor view file in my next message. The summary should be up to 2 sentences long and include the view's purpose and main C# logic. If there are comments in the file marked as notes for creating this summary, follow them strictly."

const defaultAIAnnotatePrompt_CSHTML_CS = "Create a summary for the C# code-behind file in my next message. It should be as brief as possible, without unnecessary language structures. The summary should include the class name (if applicable), brief general description of the class's purpose, and a list of declared entities. Skip private entities from listing completely.\n\nIf there are comments in the file marked as notes for creating this summary, follow them strictly. Otherwise, use the following template:\n\nClass: `<class name>`\n\nThis code-behind file provides <description of what this file is used for, up to 3 sentences>\n\nDeclarations:\n\n- `<entity name>`: <entity type>, <summary>"
...
const defaultAIAnnotatePrompt_Py3 = "Create a summary for the Python 3 source file in my next message. It should be as brief as possible, without unnecessary language structures. The summary should include full subpackage name if present and a list of declared entities. Skip private entities from listing completely.\n\nIf there are comments in the file marked as notes for creating this summary, follow them strictly. Otherwise, use the following template:\n\nSubpackage: <full subpackage name with namespace if present>\n\nThis file provides <description of what this file is used for, up to 3 sentences>\n\nDeclarations:\n\n- `<entity name>`: <entity type>, <summary>"

const defaultAIAnnotatePrompt_Py3_Short = "Create a short summary for the Python 3 source file in my next message. The summary should be up to 2 sentences long, and should include full subpackage name if present.\n\nIf there are comments in the file marked as notes for creating this summary, follow them strictly. Otherwise, use the following template:\n\nSubpackage: <full subpackage name with namespace if present>\n\n<Description of what this file is used for, up to 2 sentences>"
...
func getDefaultImplementConfigTemplate() map[string]any {
	...
	// stage 3
	result[config.K_ImplementStage3PlanningPrompt] = "Now create a list of filenames that will be changed, created, or deleted by you as a result of implementing the tasks according to your work plan. Place each filename that will be changed or created between <filename></filename> tags. If an existing file should be deleted, place that filename between <delete></delete> tags."
	result[config.K_ImplementStage3ExtraFilesPrompt] = "Below are the contents of additional source code files that may be relevant to the tasks."
	...
	result[config.K_ImplementStage4ProcessIncrPrompt] = "Implement the required code for the following file: \"###FILENAME###\". Output your changes as one or more search-and-replace blocks in following format:\n\nSEARCH>>>\n<several consecutive lines of code from file needed to show the exact location where your changes will be made>\n<<<REPLACE>>>\n<lines of code with your changes implemented that will replace the lines of code above>\n<<<DONE\n\n\n\nImportant notes:\n\n- If you believe a file requires changes that cannot be efficiently packaged into search-and-replace blocks as described above, you can output the entire file at once with code markup (```), with all required changes included.\n- Be careful with indentation: do not remove leading spaces or tabs inside search-and-replace blocks, do not accidentally replace tabs with spaces and vice versa.\n- Do not use code markup (```) inside search-and-replace blocks.\n- Reply either with search-and-replace blocks or with the entire file with the changes made.\n"
