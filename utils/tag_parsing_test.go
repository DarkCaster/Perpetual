...
func TestParseTaggedText(t *testing.T) {
	testCases := []struct {
		...
		{
			name:           "LLM multiple inline outputs with newlines",
			sourceText:     "<output>data\n</output><output>blah\n</output>",
			startTagRegex:  "(?m)\\s*<output>\\n?",
			endTagRegex:    "(?m)<\\/output>\\s*$?",
			expectedResult: []string{"data\n", "blah\n"},
		...
		{
			name:           "LLM multiple inline outputs with surrounding text",
			sourceText:     "blah<output>data</output>blah<output>extra</output>blah",
			startTagRegex:  "(?m)\\s*<output>\\n?",
			endTagRegex:    "(?m)<\\/output>\\s*$?",
