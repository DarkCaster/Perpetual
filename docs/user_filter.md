...
- **Operation Behavior**: In `doc`, `explain`, `implement`, and `report`, the filter is added to the project blacklist before the project file list is built. In `misc`, it is applied after the project file list is collected. In `annotate` and `embed`, it filters the files selected for annotation or embedding; skipped files keep their previous stored checksum state so they can be processed again later if the filter is removed.
