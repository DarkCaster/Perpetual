...
- **Operation Behavior**: In `doc`, `explain`, `implement`, and `report`, the filter is added to the project blacklist before the project file list is built. In `misc`, it is applied after the project file list is collected for file-oriented modes such as listing or checking files; it has no effect on the `misc -p` project test mode. In `annotate` and `embed`, it filters the files selected for annotation or embedding; skipped files keep their previous stored checksum state so they can be processed again later if the filter is removed. In `embed` question/search mode, the filter is also applied to the file list used for similarity search.
