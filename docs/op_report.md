...
- `-c <mode>`: Set the context saving mode used when the brief report mode runs annotation generation. Valid values are:
  - `auto` (default)
  - `off`
  - `medium`
  - `high`
...
2. **Generate a brief report and save it to a file:**
   ...
   ```sh
   Perpetual report -m brief -o project_summary.txt
   ```
