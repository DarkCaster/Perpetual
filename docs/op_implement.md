...
The `implement` operation is run in one of three modes selected with the required `-m` flag: `task`, `comment`, or `comment-fast`.
...
In `comment` mode (`-m comment`), the operation works by identifying and processing sections of your code marked with `###IMPLEMENT###` comments. These comments serve as indicators for where new code should be generated or existing code modified. Comment mode uses planning by default, so it can also modify other files, create new files, or delete files if required by the implementation.

In `comment-fast` mode (`-m comment-fast`), the operation also processes `###IMPLEMENT###` comments but **skips planning entirely**. It only modifies the files containing those comments and cannot create new files, delete files, or affect other project files. This mode saves API calls and tokens for simple, localized edits.

All modes follow a multi-stage process to ensure accurate and contextually appropriate code implementation:
...
2. **Target File Identification**  
   In task mode, `###IMPLEMENT###` comments are ignored; there are no initial target files, and relevant files are selected later according to the task instructions and project index. In comment and comment-fast modes, files containing `###IMPLEMENT###` comments are identified as initial targets for code implementation.
...
2. **Describing the Implementation**  
   ...
   - In comment or comment-fast mode, use `###IMPLEMENT###` comments in your source files to indicate where you want code to be generated or modified. This is best suited for minor edits or when you need to precisely specify the location of the code. In comment-fast mode, creating and deleting files is not possible; use comment or task mode if you need to create or delete files.  
...
- `###IMPLEMENT###`: Marks sections for code implementation in comment and comment-fast modes. You can provide detailed instructions after this comment.  
...
- `-m <mode>`: Select operation mode (required). Valid values: `task`, `comment`, `comment-fast`.  
...
2. **Planning Mode**: In task mode (`-m task`) or comment mode (`-m comment`), the LLM produces reasonings and a work plan outlining the codebase changes required. This allows subsequent stages to modify related files, create new files, and delete existing files.  
3. **No Planning Mode**: In comment-fast mode (`-m comment-fast`), the LLM is given only the target files and related context, and it is prepared to implement changes directly within those files. No work plan or reasoning is generated, and no other files are affected.

### Stage 3: Planning Changes

1. **Determine Files to Modify, Create, or Delete**: If planning is enabled (task or comment mode), query the LLM for the list of files that will be modified, created, or deleted. In comment-fast mode, this stage does not perform an LLM request and simply uses the initial target files.  
...
- **`stage2_reasonings_prompt`**: Prompt for generating detailed reasoning and a work plan when planning mode is enabled in comment mode (which is the default).
...
- **`stage2_noplanning_prompt`**: Prompt used when planning mode is disabled (comment-fast mode), requesting direct implementation without a work plan.
...
1. **Prefer Task Mode**: For most implementations, use task mode (`-m task`) and describe your requirements in a task file or via stdin. Reserve comment mode (`-m comment`) for targeted changes that benefit from planning, and comment-fast mode (`-m comment-fast`) for simple, localized edits.  
...
4. **Explicitly Request Deletions**: If obsolete files should be deleted, state this clearly in your task instructions or implementation comments and use a mode with planning (task or comment).  
