...
The `implement` operation is run in one of two modes selected with the required `-m` flag: `comment` mode or `task` mode.

In `comment` mode (`-m comment`), the operation works by identifying and processing sections of your code marked with `###IMPLEMENT###` comments. These comments serve as indicators for where new code should be generated or existing code modified. The operation follows a multi-stage process to ensure accurate and contextually appropriate code implementation.

In `task` mode (`-m task`), you provide implementation instructions directly—via standard input or from a file using the `-i` flag—thereby bypassing the search for `###IMPLEMENT###` comments. This provides an alternative approach when you prefer to specify your requirements in a task description rather than annotate the code. Task mode always uses planning and can affect any project files.
...
2. **Target File Identification**  
   In comment mode, files containing `###IMPLEMENT###` comments are identified as initial targets for code implementation. In task mode, `###IMPLEMENT###` comments are ignored; there are no initial target files, and relevant files are selected later according to the task instructions and project index.
...
2. **Marking Implementation Points**  
   - In comment mode, use `###IMPLEMENT###` comments in your source files to indicate where you want code to be generated or modified. You may also describe when related files should be created or obsolete files should be deleted, but creating and deleting files requires planning mode (`-p`).  
   - Example:
   ...
   - As an alternative to using `###IMPLEMENT###` comments, you can use task mode (`-m task`) to provide implementation instructions.
...
3. **Running the Implement Operation**  
   ...
   - **Task Mode** (`-m task`): Instructions are provided through standard input or by specifying a file with `-i`. Task mode always uses planning and can affect any project files.
...
- `-m <mode>`: Select operation mode (required). Valid values: `task`, `comment`.  
  - `task`: Implement code based on a task read from a text file or stdin (see the `-i` flag). Uses planning and can affect any project files.  
  - `comment`: Generate code marked with `###IMPLEMENT###` comments in the source code. Cannot affect other files unless using the `-p` flag.  
...
2. **No Planning Mode**: In comment mode without the `-p` flag, the LLM is given the target files and related context and is prepared to implement only the initially targeted files. File creation and deletion are not planned in this mode.  
3. **Planning Mode**: In task mode (`-m task`) or comment mode with the `-p` flag, the LLM produces reasonings and a work plan outlining the codebase changes required. This allows subsequent stages to modify related files, create new files, and delete existing files.
...
3. **Explicitly Request Deletions**: If obsolete files should be deleted, state this clearly in your task instructions or implementation comments and use planning mode (`-p` in comment mode, or task mode).  
...
8. **Use Planning When Needed**: For extensive changes, enable `-p` in comment mode or use task mode (`-m task`) for broader file modification, creation, and deletion planning.  
...
2. **Do Not Use `-p` Unless Needed**: In comment mode, you may significantly save on LLM API calls, tokens, and costs by not using the `-p` flag if you believe that the implementation will not create new files, delete files, or modify files not marked with `###IMPLEMENT###` comments.  
